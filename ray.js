class Ray {
	...
	render() {
		...
		for (let child of this.children) { //recursively render all children
			...
			try {
				child.render();
			} catch (e) {} //this is so that it doesnt throw an error
		}
	}
	...
	doesCollide(obj) { //function to take any object and process collision detection
		if (obj instanceof PolygonalBlock) {
			...
			if (dSq <= obj.r ** 2) { //only collides if normal point within arc radius
				...
				v.add(obj.pos);
				v2.add(obj.pos); //get point relative to origin from vector
			...
			if (dSq <= obj.r ** 2) { //only collides if normal point within arc radius
				...
				if (dSq <= 1e-5) { //if ray passes through center
					...
					v2 = this.dir.copy().mult(-obj.r); //^ but other direction
					vA = this.dir.heading();
					v2A = vA + PI; //angle of both vectors relative to global 0 angle
				} else {
					...
					vA = a0 + a;
					v2A = a0 - a; //angle of both vectors relative to global 0 angle
				}
				const pts = []; //may return one point or array of both
	...
	collWithLine(obj) {
		...
		const x1 = obj.pos.x,
			y1 = obj.pos.y;
		const x2 = obj.pos.x + obj.displacement.x,
			y2 = obj.pos.y + obj.displacement.y;
		const x3 = this.pos.x,
			y3 = this.pos.y;
